...
%{
    ...
    template <class T> void narray_of_numpy(narray<T> &a,PyObject *object_) {
        ...
        if(t==PyArray_FLOAT) {
            ...
            npy_intp *data = (npy_intp*)PyArray_DATA(obj);
            for(int i=0;i<n;i++) a.unsafe_at1d(i) = data[i];
        }
...
template <class T> void add(narray<T> &,narray<T> &);
%template(add) add<float>;
%template(add) add<int>;
%template(add) add<byte>;
...
template <class T,class S> void add(narray<T> &,narray<S> &,narray<S> &);
...
template <class T,class S> void pow(narray<T> &,narray<S> &,narray<S> &);
